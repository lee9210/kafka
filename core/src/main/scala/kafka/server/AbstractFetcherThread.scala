...
abstract class AbstractFetcherThread(name: String,// 线程名称
                                     ...
                                     val sourceBroker: BrokerEndPoint,// 数据源Broker地址。源Broker是指此线程要从哪个Broker上读取数据。
                                     failedPartitions: FailedPartitions,// 处理过程中出现失败的分区
                                     fetchBackOffMs: Int = 0,// 获取操作重试间隔，默认是Broker端参数replica.fetch.backoff.ms值。
                                     isInterruptible: Boolean = true,// 线程是否允许被中断
                                     val brokerTopicStats: BrokerTopicStats)  // Broker端主题监控指标//BrokerTopicStats's lifecycle managed by ReplicaManager
  ...
  protected def truncateFullyAndStartAt(topicPartition: TopicPartition,  // 要对哪个分区下副本执行截断操作
                                        offset: Long): Unit // Offset + 截断状态
